...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    let ball = new Circle(450, 300, 10, "white")
    

    class Rampsegment{
        constructor(line){
            this.line = line
            this.body = castBetween(line.object, line.target, line.hypotenuse()/3, 14)
            this.smallbody = castBetween(line.object, line.target, line.hypotenuse()/1, 7)
            this.acceleration = new Vector(ball)
        }
        draw(){
            this.line.draw()
            // this.body.draw()

            this.body = castBetween(this.line.object, this.line.target, this.line.hypotenuse()/3, 14)
            this.smallbody = castBetween(this.line.object, this.line.target, this.line.hypotenuse()/1, 7)

            this.line.object.y += (Math.random()-.5)*3
        }
    }
    let center = new Circle(550,350, 2, "red")
    let angle = 0
    let points = []

    for(let t = 0;t < 17;t++){
        let point = new Circle(center.x+(Math.cos(angle)*350), center.y+(Math.sin(angle)*250), 2, "blue")
        angle += .2
        points.push(point)
    }
    ...
    for(let t = 1;t < points.length;t++){
        let link = new LineOP(points[t], points[t-1], "#00ff00", 3)
        let rp = new Rampsegment(link)
        links.push(rp)
    }
    ...
    function main() {
        ...
        for(let t = 0;t<links.length;t++){
            // links[t].draw()
            if(links[t].body.doesPerimeterTouch(ball)){
                wet = 1
                index = t
            }
            if(links[t].smallbody.doesPerimeterTouch(ball)){
                wet = 2
                index = t
                break
            }
        ...
        if(wet == 0){
            ...
            // ball.y--
            ball.ymom += (Math.sin(links[index].line.angle())*physics.gravity)
            ball.xmom += Math.cos(links[index].line.angle()+(Math.PI*.5))*physics.gravity
        }
        ball.move()
        ball.draw()
    }
